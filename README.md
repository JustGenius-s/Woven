...
- 场景旅程：以“入景 → 识音 → 记词 → 懂句 → 会话 → 开口”贯通五十音、词汇、语法、日常对话和 AI 对练；当前旅程及完成阶段会本地保存，并同步出现在假名、词库和对练页。
...
- 内容舞台：旅程、词典、课文、新闻、视频和对练不共用列表模板；分别采用连续场景、词典焦点、长文编排、横向文章、字幕时间线和单场景舞台。
...
- 主题：跟随系统深浅色；以“宇宙蓝、雪域白、星空黑”为主色，内容平面仅在核心互动区域使用沉浸光感和轻拟物层次。
- 动效：页面与阶段切换采用状态驱动的弹性运动，圆、环、胶囊和卡片通过“起源、平衡、悬浮、吸引”的关系表达学习进度；装饰动画不做无限循环。
